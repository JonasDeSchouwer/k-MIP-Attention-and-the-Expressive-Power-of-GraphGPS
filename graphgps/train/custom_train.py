...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in tqdm(enumerate(loader)):
        ...
        # performs a deep copy of the batch
        ...
        try:
            ...
            if _is_OOM_error(e):
                logging.info(f"OOM error occurred on batch {batch}. Retrying after splitting batch. This impacts performance.")
                optimizer.zero_grad()
                if batch_accumulation > 1:
                    warnings.warn("Batch_accumulation > 1, but OOM error occurred. This is dangerous, as we need to erase the gradients of the previous batch.")
        ...
        if not get_gradients_succeeded:
            ...
            for data in data_list:
                try:
                    _true_batch, _pred_batch, loss_batch = _get_single_batch_gradients(model, Batch.from_data_list([data]), loader, batch_accumulation * len(data_list))
                    _trues.append(_true_batch)
                    _preds.append(_pred_batch)
                    losses.append(loss_batch)
                except RuntimeError as e:
                    if _is_OOM_error(e):
                        logging.info(f"OOM error occurred on element of batch. Retrying with gradient checkpointing. This impacts performance.")
            ...
            if len(_trues) == 0:
                ...
                continue
            elif len(_trues) == len(data_list):
                logging.info("After splitting, all elements of batch were processed successfully.")
            else:
...
def _get_single_batch_predictions(model, batch, loader, split):
    if split not in ['val', 'test']:
        raise ValueError(f"Split {split} was not expected. Only val and test are supported in eval epochs.")
    batch.split = split
    batch.to(torch.device(cfg.device))
