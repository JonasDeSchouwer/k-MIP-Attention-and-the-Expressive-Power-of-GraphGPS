...
class ShapeNet(InMemoryDataset):
    ...
    def download(self):
        try:
            ...
            print(f"Error downloading from {self.url}. If `shapenet.cs.stanford.edu` is offline, try to download the data from Kaggle instead (requires login): https://www.kaggle.com/datasets/mitkir/shapenet/download?datasetVersionNumber=1")
            raise e
